...
/* ═══════════════════════════════════════════════════
   TYPES
═══════════════════════════════════════════════════ */
type Category = "Drinks" | "Food" | "Snacks" | "Hygiene" | "Household";
type StockTabId = "home" | "stock" | "alerts";
type SortKey = "name_asc" | "name_desc" | "qty_low" | "qty_high" | "value";
...
const RWF = (n: number | null | undefined) =>
  `${Math.round(Number(n) || 0).toLocaleString()} RWF`;

const C: Record<Category, CategoryConfig> = {
...
const isCategory = (value: unknown): value is Category =>
  typeof value === "string" && Object.prototype.hasOwnProperty.call(C, value);

const safeCategory = (value: unknown): Category | null =>
  isCategory(value) ? value : null;

const mapInventoryRow = (row: InventoryDbRow): InventoryItem => {
  ...
  return {
    ...
    normalized_name: row.normalized_name ?? normalizeItemName(itemName),
    created_at: row.created_at ?? undefined,
  } as InventoryItem;
};

const canShowSubcategory = (cat: InventoryForm["cat"]): cat is Category => {
  if (!cat) return false;
  return Object.prototype.hasOwnProperty.call(SUBS, cat);
};

/* ═══════════════════════════════════════════════════
   SMALL SHARED COMPONENTS
═══════════════════════════════════════════════════ */
const CatIcon = ({ cat, size = 40 }: { cat?: string | null; size?: number }) => {
  const category = safeCategory(cat);
  const conf = category ? C[category] : { e: "📦", bg: "#f1f5f9" };
...
const QtyBadge = ({ qty, outLabel }: { qty: number; outLabel: string }) => {
  if (qty === 0) {
    return (
      <span style={{ background: "#fee2e2", color: "#b91c1c", padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: "0.3px" }}>
        {outLabel}
      </span>
    );
  }
...
const Pill = ({ label, active, color, onClick }: { label: string; active: boolean; color?: string; onClick: () => void }) => (
  <button
    type="button"
    onClick={onClick}
    style={{
...
const StockBar = ({ qty, max = 50 }: { qty: number; max?: number }) => {
...
const OwnerOnlyNotice = ({ message }: { message: string }) => (
  <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", color: "#9a3412", borderRadius: 14, padding: "12px 14px", display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
    <ShieldAlert size={17} />
...
const FormFields = ({
  ...
  t: (key: string) => string;
  categoryLabel: (cat: Category | string | null | undefined) => string;
  subcategoryLabel: (sub: string | null | undefined) => string;
  disabled?: boolean;
}) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 14, opacity: disabled ? 0.75 : 1 }}>
    <div>
      ...
      <input
        ...
        style={{ width: "100%", height: 46, padding: "0 14px", borderRadius: 12, border: "1.5px solid #e2e8f0", fontSize: 14, outline: "none", background: disabled ? "#f1f5f9" : "#f8fafc", boxSizing: "border-box", color: "#0f172a", fontFamily: "inherit" }}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
      <div>
        ...
        <input
          ...
          placeholder="0"
          value={vals.qty}
          onChange={(e) => onChange({ ...vals, qty: e.target.value.replace(/[^\d]/g, "") })}
          style={{ width: "100%", height: 46, padding: "0 14px", borderRadius: 12, border: "1.5px solid #e2e8f0", fontSize: 15, fontWeight: 700, outline: "none", background: disabled ? "#f1f5f9" : "#f8fafc", boxSizing: "border-box", color: "#0f172a", fontFamily: "inherit" }}
          onFocus={(e) => (e.target.style.borderColor = "#6366f1")}
          onBlur={(e) => (e.target.style.borderColor = "#e2e8f0")}
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="0"
          value={vals.price}
          onChange={(e) => onChange({ ...vals, price: e.target.value.replace(/[^\d.]/g, "") })}
          style={{ width: "100%", height: 46, padding: "0 14px", borderRadius: 12, border: "1.5px solid #e2e8f0", fontSize: 15, fontWeight: 700, outline: "none", background: disabled ? "#f1f5f9" : "#f8fafc", boxSizing: "border-box", color: "#0f172a", fontFamily: "inherit" }}
          onFocus={(e) => (e.target.style.borderColor = "#6366f1")}
          onBlur={(e) => (e.target.style.borderColor = "#e2e8f0")}
        />
        ...
        {Object.entries(C).map(([k, v]) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              type="button"
              onClick={() => onChange({ ...vals, cat: category, sub: "" })}
              style={{ flexShrink: 0, border: vals.cat === category ? "none" : `1.5px solid ${v.bg}`, background: vals.cat === category ? v.c : v.bg, color: vals.cat === category ? "#fff" : v.c, borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 5 }}
            >
              <span style={{ fontSize: 15 }}>{v.e}</span>
        ...
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
          {SUBS[vals.cat].map((s) => (
            <button
              ...
              disabled={disabled}
              type="button"
              onClick={() => onChange({ ...vals, sub: s })}
              style={{ flexShrink: 0, border: vals.sub === s ? "none" : "1.5px solid #e2e8f0", background: vals.sub === s ? "#6366f1" : "#f8fafc", color: vals.sub === s ? "#fff" : "#475569", borderRadius: 20, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer" }}
            >
              {subcategoryLabel(s)}
            </button>
          ))}
...
/* ═══════════════════════════════════════════════════
   SHEET WRAPPER
═══════════════════════════════════════════════════ */
const Sheet = ({ onClose, children }: { onClose: () => void; children: ReactNode }) => (
  <div
  ...
  >
    <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 500, maxHeight: "92dvh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(0,0,0,0.18)" }}>{children}</div>
  </div>
);
...
export default function InventoryApp() {
  ...
  const canEditInventory = isOwner;
  const canAdjustStock = Boolean(profile);
 const canAddInventory = isOwner;

 const categoryLabel = useCallback(
  (cat: Category | string | null | undefined) => {
    const category = safeCategory(cat);

    if (!category) return t("inventory.uncategorized");

    return t(`inventory.categories.${CATEGORY_KEYS[category]}`);
  },
  [t]
);
  const subcategoryLabel = useCallback(
    (sub: string | null | undefined) => (sub ? t(`inventory.subcategories.${SUBCATEGORY_KEYS[sub] || sub}`) : t("inventory.noSubcategory")),
    [t]
  );

  const [items, setItems] = useState<InventoryItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<StockTabId>("home");
  const [selected, setSelected] = useState<InventoryItem | null>(null);
  const [addOpen, setAddOpen] = useState(false);
  const [toast, setToast] = useState<ToastState>(null);

  const [form, setForm] = useState<InventoryForm>(EMPTY_FORM);
  const [editForm, setEditForm] = useState<InventoryForm | null>(null);
  ...
  const ownerOnlyMessage = t("inventory.ownerOnlyDangerAction") || "Only the owner can perform this action.";
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const nextItems = ((data ?? []) as InventoryDbRow[]).map(mapInventoryRow);
      setItems(nextItems);
      setSelected((current) => {
  ...
  const summary = useMemo(
    () => ({
      ...
      value: items.reduce((s, i) => s + Number(i.quantity || 0) * Number(i.cost_price || 0), 0),
    ...
    () =>
      Object.keys(C).map((catKey) => {
        ...
        return {
          cat,
          count: ci.length,
          units: ci.reduce((s, i) => s + i.quantity, 0),
          value: ci.reduce((s, i) => s + i.quantity * i.cost_price, 0),
          low: ci.filter((i) => i.quantity <= LOW).length,
        };
      }),
    [items]
  );
  ...
  const filteredItems = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "qty_high":
        ...
        break;
      case "value":
        result = [...result].sort((a, b) => b.quantity * b.cost_price - a.quantity * a.cost_price);
        break;
    }
  ...
  const handleAdd = useCallback(async () => {
    if (!canAddInventory) {
      showToast(t("inventory.notAllowed"), "err");
      return;
    }

    const q = Number.parseInt(form.qty, 10);
    const p = Number.parseFloat(form.price);

    if (!form.name.trim() || Number.isNaN(q) || Number.isNaN(p) || !form.cat || !form.sub) {
      showToast(t("inventory.fillAllFieldsDetailed"), "err");
      return;
    }

    if (q < 0 || p < 0) {
      showToast(t("inventory.quantityCostCannotBeNegative"), "err");
      return;
    }
  ...
  }, [canAddInventory, form, showToast, t]);

  const handleUpdate = useCallback(async () => {
    if (!canEditInventory) {
      showToast(ownerOnlyMessage, "err");
      return;
    }

    if (!editForm || !selected) return;

    const q = Number.parseInt(editForm.qty, 10);
    const p = Number.parseFloat(editForm.price);

    if (!editForm.name.trim() || Number.isNaN(q) || Number.isNaN(p) || !editForm.cat || !editForm.sub) {
      showToast(t("inventory.fillAllFields"), "err");
      return;
    }
  ...
  }, [canEditInventory, editForm, ownerOnlyMessage, selected, showToast, t]);
  ...
  const handleAdjust = useCallback(
    async (delta: number) => {
      ...
      if (nq < 0) {
        showToast(t("inventory.quantityCannotBeNegative"), "err");
        return;
      }
    ...
    [canAdjustStock, selected, showToast, t]
  );
  ...
  const handleDelete = useCallback(
    ...
    [canDeleteInventory, ownerOnlyMessage, showToast, t]
  );
  ...
  const openItem = useCallback((item: InventoryItem) => {
    ...
    setEditForm({
      ...
      price: String(item.cost_price),
      cat: safeCategory(item.category) || "",
    ...
    return (
      <div>
        <div style={{ background: "#0f172a", padding: "0 20px 28px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, margin: "0 0 16px" }}>
              {summary.total} {t("inventory.products")} · {summary.units.toLocaleString()} {t("inventory.units")}
            </p>

            {summary.out + summary.low > 0 ? (
              <button type="button" onClick={() => setTab("alerts")} style={{ background: "rgba(239,68,68,0.18)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", width: "100%", textAlign: "left" }}>
                <AlertTriangle size={15} color="#fca5a5" />
                <span style={{ color: "#fca5a5", fontSize: 13, fontWeight: 600, flex: 1 }}>
                  {summary.out > 0 && `${summary.out} ${t("inventory.outOfStockLower")}`}
                  {summary.out > 0 && summary.low > 0 && " · "}
                  {summary.low > 0 && `${summary.low} ${t("inventory.runningLow")}`}
                </span>
        ...
        <div style={{ padding: "16px 16px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            { label: t("inventory.products"), value: summary.total, color: "#6366f1", bg: "#eef2ff", icon: <Boxes size={15} color="#6366f1" />, target: "stock" as StockTabId },
            { label: t("inventory.totalUnits"), value: summary.units.toLocaleString(), color: "#0284c7", bg: "#e0f2fe", icon: <Package size={15} color="#0284c7" />, target: "stock" as StockTabId },
            { label: t("inventory.outOfStock"), value: summary.out, color: "#dc2626", bg: "#fee2e2", icon: <AlertTriangle size={15} color="#dc2626" />, target: "alerts" as StockTabId },
            { label: t("inventory.lowStock"), value: summary.low, color: "#d97706", bg: "#fef3c7", icon: <Bell size={15} color="#d97706" />, target: "alerts" as StockTabId },
          ].map((card) => (
            <div key={card.label} onClick={() => setTab(card.target)} style={{ background: "#fff", borderRadius: 16, padding: "14px 16px", border: "1px solid #f1f5f9", cursor: "pointer" }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: card.bg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>{card.icon}</div>
        ...
        <div style={{ margin: "14px 16px 0", background: "#fff", borderRadius: 16, padding: 16, border: "1px solid #f1f5f9" }}>
          ...
          {catBreakdown.map(({ cat, value, count, low, units }) => {
            ...
            return (
              <div key={cat} style={{ marginBottom: 13, cursor: "pointer" }} onClick={() => { setCatFilter(cat); setTab("stock"); }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div>
                      ...
                      <span style={{ fontSize: 11, color: "#94a3b8", marginLeft: 6 }}>
                        {count} {t("inventory.items")} · {units} {t("inventory.units")}
                      </span>
                    </div>
        ...
        <div style={{ margin: "14px 16px 24px", display: "flex", gap: 10 }}>
          <button type="button" onClick={() => setTab("stock")} style={{ flex: 1, background: "#0f172a", color: "#fff", border: "none", borderRadius: 14, padding: "14px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            <Package size={15} /> {t("inventory.viewAllStock")}
          </button>
         <button
  type="button"
  disabled={!canAddInventory}
  onClick={() => canAddInventory && setAddOpen(true)} style={{ flex: 1, background: "#059669", color: "#fff", border: "none", borderRadius: 14, padding: "14px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
  ...
  const StockTab = () => {
    const SORT_LABELS: Record<SortKey, string> = {
      name_asc: t("inventory.sortNameAsc"),
      name_desc: t("inventory.sortNameDesc"),
      qty_low: t("inventory.sortQtyLowFirst"),
      qty_high: t("inventory.sortQtyHighFirst"),
      value: t("inventory.sortValueHighFirst"),
    };
    ...
    return (
      <div style={{ padding: "14px 14px 0" }}>
        <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", padding: "0 14px", display: "flex", alignItems: "center", gap: 10, marginBottom: 12, height: 48 }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("inventory.searchByNameCategorySubcategory")} style={{ flex: 1, border: "none", outline: "none", fontSize: 14, background: "transparent", color: "#0f172a", fontFamily: "inherit" }} />
          {search ? (
            <button type="button" onClick={() => setSearch("")} style={{ background: "#f1f5f9", border: "none", width: 24, height: 24, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, marginBottom: 12 }}>
          <Pill label={t("inventory.all")} active={catFilter === "All"} onClick={() => setCatFilter("All")} />
          {Object.entries(C).map(([k, v]) => {
            const category = k as Category;

            return (
              <button key={category} type="button" onClick={() => setCatFilter(catFilter === category ? "All" : category)} style={{ flexShrink: 0, border: catFilter === category ? "none" : `1.5px solid ${v.bg}`, background: catFilter === category ? v.c : v.bg, color: catFilter === category ? "#fff" : v.c, borderRadius: 24, padding: "7px 13px", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <div style={{ position: "relative" }}>
            <button type="button" onClick={() => setShowSort((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 6, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: "7px 12px", fontSize: 12, color: "#475569", fontWeight: 600, cursor: "pointer" }}>
              ...
              <>
                ...
                <div style={{ position: "absolute", right: 0, top: "calc(100% + 4px)", background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden", zIndex: 60, minWidth: 170, boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
                  {Object.entries(SORT_LABELS).map(([k, v]) => {
                    ...
                    return (
                      <button key={key} type="button" onClick={() => { setSort(key); setShowSort(false); }} style={{ width: "100%", padding: "10px 14px", background: sort === key ? "#f8fafc" : "#fff", border: "none", textAlign: "left", fontSize: 13, color: sort === key ? "#0f172a" : "#475569", fontWeight: sort === key ? 700 : 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        {v} {sort === key && <Check size={13} color="#059669" />}
                      </button>
                    );
                  })}
                </div>
        ...
        {filteredItems.length === 0 ? (
          ...
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f1f5f9", overflow: "hidden", marginBottom: 8 }}>
            {filteredItems.map((item, i) => (
              <div key={item.id} style={{ width: "100%", background: "none", border: "none", borderBottom: i < filteredItems.length - 1 ? "1px solid #f8fafc" : "none", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#fafafa")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
                <button type="button" onClick={() => openItem(item)} style={{ flex: 1, minWidth: 0, background: "none", border: "none", padding: 0, display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: "#94a3b8", margin: 0 }}>
                      {subcategoryLabel(item.subcategory)} · <span style={{ color: "#64748b", fontWeight: 500 }}>{RWF(item.cost_price)}</span>
                    </p>
                ...
                {canDeleteInventory && (
                  <button type="button" onClick={() => void handleDelete(item.id)} title={t("inventory.deleteItem")} style={{ width: 34, height: 34, borderRadius: 11, background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
          ...
          {outItems.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #fee2e2", overflow: "hidden" }}>
                {outItems.map((item, i) => (
                  <button key={item.id} type="button" onClick={() => openItem(item)} style={{ width: "100%", background: "none", border: "none", borderBottom: i < outItems.length - 1 ? "1px solid #fef2f2" : "none", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left" }}>
          ...
          {lowItems.length > 0 && (
            <div>
              ...
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #fde68a", overflow: "hidden" }}>
                {lowItems.map((item, i) => (
                  <button key={item.id} type="button" onClick={() => openItem(item)} style={{ width: "100%", background: "none", border: "none", borderBottom: i < lowItems.length - 1 ? "1px solid #fffbeb" : "none", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left" }}>
                    <CatIcon cat={item.category} size={40} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.item_name}</p>
                      <p style={{ fontSize: 11, color: "#94a3b8", margin: 0 }}>{categoryLabel(item.category)} · {subcategoryLabel(item.subcategory)}</p>
  ...
  const AddSheet = () => (
    <Sheet onClose={() => setAddOpen(false)}>
      ...
      <div style={{ padding: "12px 20px 16px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button type="button" onClick={() => setAddOpen(false)} style={{ width: 34, height: 34, borderRadius: 10, background: "#f1f5f9", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <X size={15} color="#64748b" />
        </button>
      </div>

      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
        <FormFields vals={form} onChange={setForm} t={t} categoryLabel={categoryLabel} subcategoryLabel={subcategoryLabel} disabled={!canAddInventory} />
      </div>

      <div style={{ padding: "14px 20px", borderTop: "1px solid #f1f5f9" }}>
        <button disabled={!canAddInventory} type="button" onClick={() => void handleAdd()} style={{ width: "100%", height: 52, background: canAddInventory ? "#059669" : "#94a3b8", color: "#fff", border: "none", borderRadius: 16, fontSize: 15, fontWeight: 800, cursor: canAddInventory ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, letterSpacing: "0.2px" }}>
  ...
  const DetailSheet = () => {
    ...
    const selectedCategory = safeCategory(selected.category);
    const conf = selectedCategory ? C[selectedCategory] : { e: "📦", c: "#475569", bg: "#f1f5f9" };
        ...
        <div style={{ padding: "10px 20px 16px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
              {selectedCategory && <span style={{ fontSize: 10, background: conf.bg, color: conf.c, padding: "2px 9px", borderRadius: 20, fontWeight: 700 }}>{categoryLabel(selectedCategory)}</span>}
          ...
          <button type="button" onClick={() => setSelected(null)} style={{ width: 34, height: 34, borderRadius: 10, background: "#f1f5f9", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <X size={15} color="#64748b" />
          </button>
        </div>

        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          {!isOwner && <OwnerOnlyNotice message={ownerOnlyMessage} />}
          ...
          <div style={{ background: "#f8fafc", borderRadius: 16, padding: "14px 16px", marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
            <button disabled={!canAdjustStock} type="button" onClick={() => void handleAdjust(-1)} style={{ width: 46, height: 46, borderRadius: 13, background: "#fff", border: "1.5px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: canAdjustStock ? "pointer" : "not-allowed", flexShrink: 0 }}>
            ...
            <button disabled={!canAdjustStock} type="button" onClick={() => void handleAdjust(1)} style={{ width: 46, height: 46, borderRadius: 13, background: canAdjustStock ? "#0f172a" : "#94a3b8", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: canAdjustStock ? "pointer" : "not-allowed", flexShrink: 0 }}>
          ...
          {canEditInventory && (
            <>
              <button type="button" onClick={() => setShowEdit((v) => !v)} style={{ width: "100%", background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", marginBottom: showEdit ? 0 : 12 }}>
              ...
              {showEdit && editForm && (
                <div style={{ background: "#f8fafc", borderRadius: 14, padding: 16, marginBottom: 12, marginTop: 4 }}>
                  <FormFields vals={editForm} onChange={setEditForm} t={t} categoryLabel={categoryLabel} subcategoryLabel={subcategoryLabel} />
                  <button type="button" onClick={() => void handleUpdate()} style={{ width: "100%", height: 48, background: "#0f172a", color: "#fff", border: "none", borderRadius: 13, fontSize: 14, fontWeight: 800, cursor: "pointer", marginTop: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          {canDeleteInventory && (
            <button type="button" onClick={() => void handleDelete(selected.id)} style={{ width: "100%", height: 48, background: "#fef2f2", color: "#dc2626", border: "1.5px solid #fecaca", borderRadius: 13, fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8 }}>
 ...
 const TABS: Array<
  | { id: StockTabId; icon: ReactNode; label: string; badge?: number; special?: false }
  | { id: "add"; icon: ReactNode; label: string; special: true }
> = [
  ...
  const headerConfig: Record<StockTabId, { title: string; dark: boolean }> = {
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#f1f5f9", fontFamily: "-apple-system, 'SF Pro Display', 'Segoe UI', system-ui, sans-serif", maxWidth: 500, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      <div style={{ background: hc.dark ? "#0f172a" : "#fff", padding: "14px 20px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, borderBottom: hc.dark ? "none" : "1px solid #f1f5f9" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          <button type="button" onClick={() => navigate("/dashboard")} title={t("settings.backToDashboard")} aria-label={t("settings.backToDashboard")} style={{ width: 36, height: 36, borderRadius: 11, background: hc.dark ? "rgba(255,255,255,0.12)" : "#f1f5f9", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
        ...
        {tab === "stock" &&  (
         <button type="button"onClick={() => canAddInventory && setAddOpen(true)} style={{ width: 36, height: 36, borderRadius: 11, background: "#0f172a", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <Plus size={17} color="#fff" />
          </button>
        )}
      ...
      <div style={{ background: "#fff", borderTop: "1px solid #f1f5f9", padding: "6px 0 10px", display: "flex", alignItems: "flex-end", justifyContent: "space-around", flexShrink: 0 }}>
        {TABS.map((tabItem) => {
          if (tabItem.special) {
            return (
              <button key={tabItem.id} type="button" onClick={() => setAddOpen(true)} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: 0, paddingBottom: 2 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 600, color: "#059669", marginTop: 2 }}>{tabItem.label}</span>
              </button>
            );
          }
          ...
          return (
            <button key={tabItem.id} type="button" onClick={() => {
  if (tabItem.id !== "add") setTab(tabItem.id);
              ...
              <div style={{ color: active ? "#0f172a" : "#94a3b8", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {tabItem.icon}
                {"badge" in tabItem && Number(tabItem.badge) > 0 && (
                  <span style={{ position: "absolute", top: -5, right: -7, minWidth: 16, height: 16, background: "#dc2626", borderRadius: 8, fontSize: 9, fontWeight: 900, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 3px", border: "2px solid #fff" }}>
                    {Number(tabItem.badge) > 9 ? "9+" : tabItem.badge}
                  </span>
                )}
              </div>
              <span style={{ fontSize: 10, fontWeight: active ? 800 : 500, color: active ? "#0f172a" : "#94a3b8" }}>{tabItem.label}</span>
