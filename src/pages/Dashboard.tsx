...
const DashboardPage = () => {
  ...
  const fetchDashboardStats = useCallback(async () => {
    ...
    try {
      ...
      // ── All-time cash sales (sale_price already = unit_price × quantity)
      ...
      <div className="mx-auto w-full max-w-5xl space-y-4">
        ...
        {loadingStats ? (
          ...
          <>
            ...
            {/* ── Owner-only: Total revenue breakdown card ── */}
            {isOwner && (
            ...
            {!isOwner && (
              <div className="rounded-[24px] bg-gradient-to-br from-blue-50 to-indigo-50 p-6 ring-1 ring-blue-200">
                <div className="flex items-start gap-3">
                  <ShieldAlert size={20} className="mt-0.5 flex-shrink-0 text-blue-600" />
                  <div className="flex-1">
                    <h3 className="font-semibold text-blue-900">{t("dashboard.employeeAccess")}</h3>
                    <p className="mt-1 text-sm text-blue-700">{t("dashboard.employeeNotice")}</p>
                  </div>
                </div>
              </div>
            )}
